...
export default {
  ...
  data() {
    return {
      graphOptions: {
        ...
        defaultNodeColor: "rgba(0,0,0,0.06)",    //默认的节点背景颜色
  ...
  methods: {
    ...
    async getNetData(){
      ...
      await axios.get('http://localhost:8080/api/node').then(
          response =>{
            data.nodes = response.data
            console.log("1-1 node 节点获取完成")
          },
          error => {
      ...
      await axios.get('http://localhost:8080/api/link').then(
          response =>{
            data.links = response.data
            console.log("1-2 link 节点获取完成")
          },
          error => {
            console.log('请求失败了',error.message)
          }
